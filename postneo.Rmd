...
fit.post4 <- coxreg(Surv(enter, exit, event) ~ margTemp.1 + margTemp.2
                   + expTemp.3 + sex + 
                         parity + I(year - 1920), data = winter)
##dr <- drop1(fit.post, test = "Chisq")
ltx2(fit.post, caption = "Postneonatal mortality, winter. Reduced model.", label = "tab:allpost")
```
...
```{r allpostTfullred, results = "asis", cache = TRUE}
##post$expTemp <- post$eexpTemp.1
##post$margTemp <- post$extemp.1
winter <- post[!post$summer, ]
fit.post <- coxreg(Surv(enter, exit, event) ~ Temp.1 + Temp.2 + Temp.4 
                   + sex + socst +
                        parity + I(year - 1920), data = winter)
##dr <- drop1(fit.post, test = "Chisq")
ltx2(fit.post, caption = "Postneonatal mortality, temperature, winter. Reduced model.", label = "tab:allpostTfullred")
```
